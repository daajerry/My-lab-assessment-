...
function initScrollReveal() {
  ...
  if (!("IntersectionObserver" in window)) {
    // No observer support — elements are visible by default via CSS, nothing to do.
    return;
  }

  // Only now do we opt elements into the hidden starting state, since we've
  // confirmed we're able to reveal them again.
  ...
  // Safety net: if something prevents the observer from ever firing for a
  // given element (odd layout, browser quirk, etc.), don't leave it stuck
  // invisible forever.
  window.setTimeout(() => {
